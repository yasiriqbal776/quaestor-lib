...
var polisjs = require('polisjs-lib');
...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new poliscore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new poliscore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = poliscore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new poliscore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function poliscoreTest() {
      block1 = poliscore.Block.fromString(blockData);
    }

    function polisJSTest() {
      block2 = polisjs.Block.fromHex(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('poliscore', poliscoreTest, {maxTime: maxTime});
    suite.add('polisjs', polisJSTest, {maxTime: maxTime});
